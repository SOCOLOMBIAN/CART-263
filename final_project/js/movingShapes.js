class MovingShapes {
    constructor(container, shapesData, options = {}) {
      ...
      // equence and player interaction
      this.activeSequence = [];
      this.playerSequence = [];
      ...
      for (let typeIndex = 0; typeIndex < this.shapesData.length; typeIndex++) {
        for (let i = 0; i < this.options.count; i++) {
          ...
          const shape = {
            ...
            size: size,
            speedX: (Math.random() - 0.5) * this.randomRange(this.options.speed.min, this.options.speed.max),
            speedY: (Math.random() - 0.5) * this.randomRange(this.options.speed.min, this.options.speed.max),
    ...
    animate() {
      ...
      for (const shape of this.shapes) {
        ...
        // Bounce off edges
        if (shape.x < 0 || shape.x > this.canvas.width - shape.size) {
          shape.speedX *= -1;
          shape.x = Math.max(0, Math.min(this.canvas.width - shape.size, shape.x));
        }
        
        if (shape.y < 0 || shape.y > this.canvas.height - shape.size) {
          shape.speedY *= -1;
          shape.y = Math.max(0, Math.min(this.canvas.height - shape.size, shape.y));
        }
    ...
    reset() {
      ...
      for (const shape of this.shapes) {
        shape.speedX = (Math.random() - 0.5) * this.randomRange(this.options.speed.min, this.options.speed.max);
        shape.speedY = (Math.random() - 0.5) * this.randomRange(this.options.speed.min, this.options.speed.max);
